...
func generateFile(gen *protogen.Plugin, file *protogen.File) {
	...
	g.P("  idx: number,")
	g.P("): string {")
	// Each seps[...] is read into a local and checked before use: consumers
	// compile this with noUncheckedIndexedAccess, where an index expression is
	// string | undefined.
	g.P("  let rest = value.startsWith(prefix) ? value.slice(prefix.length) : value;")
	g.P("  for (let i = 0; i < idx; i++) {")
	g.P("    const sep = seps[i];")
	g.P("    if (sep === undefined) return \"\";")
	g.P("    const at = rest.indexOf(sep);")
	g.P("    if (at < 0) return \"\";")
	g.P("    rest = rest.slice(at + sep.length);")
	g.P("  }")
	g.P("  const sep = seps[idx];")
	g.P("  // Past the last separator this is the final wildcard, which takes the")
	g.P("  // remainder. A missing separator earlier means the name is malformed, and")
	g.P("  // returning what's left matches SplitMultiWildcard in the Go runtime.")
	g.P("  if (sep === undefined) return rest;")
	g.P("  const at = rest.indexOf(sep);")
